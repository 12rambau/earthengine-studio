...
The account menu uses [`vue3-google-signin`](https://www.npmjs.com/package/vue3-google-signin), the Vue wrapper for Google Identity Services' OAuth token model, to request `openid email profile` and read-only access to the current account's Google Cloud project list. It retains the resulting access token, profile, and selected project only in memory for the current browser session.

1. Create a Google OAuth 2.0 **Web application** client in the Google Cloud console and configure the consent screen.
2. Enable the **Cloud Resource Manager API** in the project that owns the OAuth client.
3. Add each application origin, such as `http://localhost:3000`, to the client's authorized JavaScript origins.
4. Copy `.env.example` to `.env.local` and set `VITE_GOOGLE_CLIENT_ID` to the web client ID.
5. Sign in again or use **Change Google account** to grant the additional `cloud-platform.read-only` consent scope.
